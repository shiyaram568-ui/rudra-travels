...
(function() {
  ...
  function animateValue(obj, start, end, duration) {
    ...
    const step = (timestamp) => {
      ...
      obj.textContent = formatINR(currentVal);
      if (progress < 1) {
        window.requestAnimationFrame(step);
      } else {
        obj.textContent = formatINR(end);
      }
    };
    window.requestAnimationFrame(step);
  }
  ...
  function displayFareEstimate(result, from, to) {
    ...
    // Update Visual Route Map
    document.getElementById('visual-pickup').textContent = from;
    document.getElementById('visual-drop').textContent = to;
    ...
    // Animate the total fare
    const totalElem = document.getElementById('est-total');
    animateValue(totalElem, 0, result.totalFare, 1500);
    ...
    // User will manually click WhatsApp button when ready
  }
